...
This repository provides information for those interested in running the COVID-19 Vulnerability Index on their own data.  We provide the index as a pretrained model implemented in Python.

The CVIndex model utilizes only a few fields, which can be extracted from administrative claims or electronic medical records.  The data requirements have intentionally been kept very limited in order to facilitate rapid implementation, while still providing good predictive power.   ClosedLoop is also offering a free, hosted version of the CV19 Index that uses additional data and provides better accuracy. For more information, see http://cv19index.com

## Install

CV19 Index can be installed from [PyPI](https://pypi.org/project/shap):
...
These wheel files are for Python 3.7.  If you have a different Python version and would like prebuilt binaries, try https://www.lfd.uci.edu/~gohlke/pythonlibs/ and create a GitHub issue. 
...
The model requires at least 6 months of claims history, so only those members iwth at least 6 months of prior history should be included.  It is not necessary that they have any claims during this period.
...
* __personId__ - An identifier for each person.  It is only used as a link into the claims table and to identify individuals in the output file.
* __age__ - Current age in years, specified as an integer
* __gender__ - Gender.  This can either be a string column containing the values 'male' or 'female', or it can be an integer columns containing the nubmers 0 and 1.  For integer columns, 0 is female and 1 is male.  Only binary genders are supported.  
...
* __personId__ - An identified that should match with a `personId` from the demographics table.
...
If you have installed the CV19 Index through PyPI package, it will create an executable that you can run.  The following command, if run from the root directory of the GitHub checkout will run the example data and put results at `examples/predictions.csv`.

_Note: The `-a 2018-12-31` is only needed because the example data is from 2018.  If you are using curretn data you can omit this argument. 

```bash
cv19index -a 2018-12-31 examples/demographics.csv examples/claims.csv examples/predictions.csv
```

Alternative, you can run it from Python directly:
